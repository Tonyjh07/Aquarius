...
（M5 进行中）：`uigui`（Gio 悬浮球 GUI，D43/§15——六面桥接 + 渲染状态机 + 窗口壳骨架已落位）；
...
| `Presenter` / `Prompter` | repl（行式）+ uitui（TUI，D33）+ uigui（GUI 悬浮球，D43/§15） | repl **M0 ✓** / uitui **M4 ✓** / uigui M5（骨架 ✓） | 收集器 / 脚本队列 |
